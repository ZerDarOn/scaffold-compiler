...
| 8.1 四组合端到端矩阵 | IN_PROGRESS | 四组合事务链通过；真实 M-01 冻结安装、代码质量和 HTTP 门通过，PostgreSQL、Docker 与发布包入口待接入 |
...
- Tests：生成器 131 tests passed、121 subtests passed；真实 M-01 的静态安全、冻结安装、Python syntax、Ruff、mypy、pytest、应用启动、live/ready 和 OpenAPI 全部 PASS；验证环境位于 W、拒绝链接/候选内路径，且外部检查后候选摘要复核通过；M-03 隔离 PostgreSQL/Alembic/SQL/HTTP readiness、M-04 Compose config 通过。
- Next：为 Task 8.1 接入 PostgreSQL/Docker 运行时验证和发布包入口；Task 4.5 的真实镜像/容器门等待 Docker 守护进程可用。
