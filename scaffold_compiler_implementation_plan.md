...
- Done：V1 的 Phases 0–8 全部完成；V2 增加通用配方架构、C11/CMake 参考配方、精确 cleanup 恢复，以及仅生成严格配置的交互式 `init`。生成、验证、原子 Finalize、失败恢复与外部监督自清理形成闭环；M-01 至 M-04 均通过真实发布验收。
...
- Next：在下一个发布候选中对 `init → preview → run` 执行 Windows/Linux 发布包验收；后续新增能力必须作为受信任配方和蓝图进入同等强度的组合矩阵。
